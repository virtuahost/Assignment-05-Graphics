...
void draw() {
  ...
  pushMatrix();   // to ensure that we can restore the standard view before writing on the canvas
    ...
    translate(width/2,height/6,dz); // puts origin of model at screen center and moves forward/away by dz
  ...
  noStroke(); drawKeyFrame();//fill(yellow); P.drawClosedLoop(); fill(orange); Q.drawClosedLoop();
  if(threeDMode)
  {
  ...
  if(deleteMode) {fill(red);text("DeleteMode",100,100); }
  else if(selectMode) { fill(magenta);text("SelectMode",100,100); }
  else if(threeDMode){fill(cyan);text("3D Drawing",100,100);}
  else { fill(green);text("AddMode",100,100); }
...
String title ="Homework 5", name ="Deep Ghosh",
